...
def song(client, message):
    ...
    try:
        ...
        m.edit(
            "INCOMPLETE Command /n/n eg:-/song hope or ytlink"
        )
        print(str(e))
        return
    m.edit("⏬ UPLOADING....")
    try:
        ...
        rep = '📓UPLODED FROM PRIVATE CLOUD '
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(
        message.chat.id, f"ACCESSING 📡.."
    )
    if not urlissed:
        await pablo.edit("🖇️REPORT @Arshkottu")
        return
    ...
    capy = f"""
[{thum}]({mo})
👋{message.from_user.mention} HAI BROO THIS VIDEO IS 720p
"""
